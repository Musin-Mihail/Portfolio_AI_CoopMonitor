...
- **Infrastructure:** Docker, MinIO (S3 Compatible Storage), MediaMTX
...
- [.NET SDK 10.0+](https://dotnet.microsoft.com/download) (для бэкенда)
...
В папке `docker` находится конфигурация для MinIO (хранилище файлов), AI-сервиса, сервера потокового видео (MediaMTX) и эмуляции камер.
...
> **Примечание:** При запуске автоматически отработает скрипт `create_buckets.sh`, который создаст необходимые бакеты в MinIO (`raw-video`, `reports`, `ai-models`, `user-uploads` и др.).

**Доступные сервисы:**

- **MinIO Console:** [http://localhost:9001](http://localhost:9001)
- **AI Service API:** [http://localhost:5001](http://localhost:5001)
- **MediaMTX (HLS потоки):** [http://localhost:8888](http://localhost:8888) (RTSP доступен на порту 8554)
...
│   ├── CoopMonitor.sln  # Файл решения (Solution)
│   └── CoopMonitor.API/ # Контроллеры, DTO, настройки
│       └── Data/        # Контекст базы данных (CoopContext)
├── docker/              # Инфраструктура
│   ├── docker-compose.yml # Оркестрация MinIO, AI сервиса, MediaMTX и камер
│   ├── mock_cameras/    # Скрипты и видео для симуляции потоков с камер
